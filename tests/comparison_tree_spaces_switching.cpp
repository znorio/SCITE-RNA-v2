...
void generate_sciterna_results(std::string const& path = "", int n_tests = 100, const std::string& pathout = "",
    ...
    std::filesystem::create_directories(pathout + "sciterna_dist_matrix");
    std::filesystem::create_directories(pathout + "sciterna_mut_indicator");
...
int main() {
    ...
    std::vector<std::vector<std::string>> tree_spaces = {{"c", "m"}}; // {"m"}, {"c"}, {"c", "m"},  {"m", "c"}
    bool flipped = false;
    ...
    for (const auto& space : tree_spaces) {
        for (int i = 0; i < n_cells_list.size(); ++i) {
            ...
            std::string path = "./" + std::to_string(n_cells) + "c" + std::to_string(n_mut) + "m";
