...
int main() {
    ...
    for (const auto& clone : clones) {
        for (int i = 0; i < n_cells_list.size(); ++i) {
            ...
            std::string path = "../data/simulated_data_geom/" + std::to_string(n_cells) + "c" + std::to_string(n_mut) + "m" + clone +"/";
//            std::string path = "/cluster/work/bewi/members/znorio/data/simulated_data/" + std::to_string(n_cells) + "c" + std::to_string(n_mut) + "m" + clone +"/";
            ...
            generate_sciterna_simulation_results(path, pathout, n_tests, tree_space, flipped_mutation_direction, n_mut, n_rounds, save_runtimes);
        }
    }
    return 0;
}