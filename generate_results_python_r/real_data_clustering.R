...
for (clone in clones) {
    ...
    memb_pred_phylinsic <- cutree(hc_phylinsic, k = clone)
    cluster_phylinsic <- DENDRO.cluster(dist_phylinsic, plot=FALSE, type="phylogram")
    parent_vec_phylinsic <- merge.to.parent(cluster_phylinsic$merge)
    write.table(memb_pred_phylinsic, file.path(base_path, "results", sample, "phylinsic", "phylinsic_clones", paste0("phylinsic_clones_", clone, ".txt")), row.names=FALSE, col.names=FALSE)
}