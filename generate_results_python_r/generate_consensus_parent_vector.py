...
def main():
    ...
    parser.add_argument("--simulated", type=bool, help="Whether the folder contains simulated or real data.", default=True)
    parser.add_argument("--model", type=str, help="Model used for the bootstrap samples.", default="sciterna")
    parser.add_argument("--n_samples", type=int, help="Number of simulated samples to process.", default=20)
    parser.add_argument("--round", type=int, help="Which round to use. Each round updates optimized SNV specific and global parameters like dropout probabilities", default=2)
    ...
    for s in range(n_samples):
        ...
        for test in range(n_bootstrap):
            ...
            path_mut_loc = os.path.join(path, f"{model}_mutation_location",
                                        f"{model}_mutation_location_{round}r{test}.txt")
            path_selected = os.path.join(path, f"{model}_selected_loci", f"{model}_selected_loci_{round}r{test}.txt")

            if not os.path.exists(path_parent) or not os.path.exists(path_mut_loc) or not os.path.exists(
                    path_selected):
                continue
