...
generate.parent.vec <- function(base_path, n.tests=10, clones=5){
  ...
  # dir.create(file.path(base_path, "cardolino", "cardolino_clones"), recursive = TRUE)
  dir.create(file.path(base_path, "sciterna", "sciterna_clustering_parent_vec"), recursive = TRUE)
  ...
  for (i in 0:(n.tests-1)){
    ...
    sciterna_genotype <- read.str.matrix(file.path(base_path, sprintf("sciterna/sciterna_genotype/sciterna_genotype_2r%d.txt", i)))
    ...
    dist_sciterna <- dist(sciterna_genotype) # use same method for DENDRO and SClineager
    hc_sciterna <- hclust(dist_sciterna, method='ward.D')
    memb_pred_sciterna <- cutree(hc_sciterna, k = clones)
    cluster_sciterna <- DENDRO.cluster(dist_sciterna, plot=FALSE, type="phylogram")
    parent_vec_sciterna <- merge.to.parent(cluster_sciterna$merge)

    write.table(parent_vec_sciterna, file.path(base_path, sprintf("sciterna/sciterna_clustering_parent_vec/sciterna_clustering_parent_vec_%d.txt", i)), row.names=FALSE, col.names=FALSE)
    write.table(memb_pred_sciterna, file.path(base_path, sprintf("sciterna/sciterna_clones/sciterna_clones_%d.txt", i)), row.names=FALSE, col.names=FALSE)

    # start_time_cardelino <- Sys.time()
    # input_data = list('A' = alt, 'D' = coverage)
    # rownames(input_data$A) <- rownames(input_data$D) <- paste0("SNV", seq_len(nrow(input_data$A)))
    # colnames(input_data$A) <- colnames(input_data$D) <- paste0("cell", seq_len(ncol(input_data$A)))
    # AF <- as.matrix(input_data$A / input_data$D)
    #
    # # p = pheatmap::pheatmap(AF, cluster_rows=FALSE, cluster_cols=FALSE,
    # #                show_rownames = TRUE, show_colnames = TRUE,
    # #                labels_row='SNVs', labels_col='Cells',
    # #                angle_col=0, angle_row=0)
    #
    # input_data$A
    # assignments <- clone_id(input_data$A, input_data$D, Config=NULL, n_clone = clones)
    #
    # names(assignments)
    # prob_heatmap(assignments$prob)
    # df <- assign_cells_to_clones(assignments$prob)
    # table(df$clone)
    # write.table(as.integer(gsub("Clone", "", df$clone)), file.path(base_path, sprintf("cardolino/cardolino_clones/cardolino_clones_%d.txt", i)),
    #             row.names=FALSE, col.names=FALSE)
    #
    # end_time_cardelino <- Sys.time()
    # runtime_cardelino <- end_time_cardelino - start_time_cardelino
  ...
  #   write.table(cardolino_runtimes, file.path(base_path, "cardolino/cardolino_runtimes.txt"), row.names = FALSE, col.names = FALSE)
}
